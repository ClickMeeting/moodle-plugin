...
if (! $clickmeetings = get_all_instances_in_course('clickmeeting', $course)) {
    notice(get_string('noclickmeetings', 'clickmeeting'), new moodle_url('/course/view.php', array('id' => $course->id)));
}
...
foreach ($clickmeetings as $clickmeeting) {
    if (!$clickmeeting->visible) {
        $link = html_writer::link(
            new moodle_url('/mod/clickmeeting.php', array('id' => $clickmeeting->coursemodule)),
    ...
    if ($course->format == 'weeks' or $course->format == 'topics') {
        $table->data[] = array($clickmeeting->section, $link);
    } else {
